...
- 2026-07-27: **Assets de Previsualización en README.** Se añadió un archivo GIF altamente optimizado de solo 509 KB (`text_preview.gif`) dentro de `/assets` que muestra un modelado interactivo y fiel de 1100 partículas formando "HOLA" en tipografía serif elegante junto al icono de la mano saludando ("👋"). Las físicas interactivas de hover/viento y la simulación del puntero se implementaron mediante un script generador en Python Pillow, garantizando máxima compatibilidad en GitHub, fluidez visual y un loop sin saltos. Se eliminaron los antiguos assets SVG para evitar problemas de renderizado de scripts en el visor de GitHub.
