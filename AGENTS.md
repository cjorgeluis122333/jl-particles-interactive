...
- Vercel should be connected to the repository root (`./`). The root `vercel.json` explicitly selects the Vite preset, runs `npm run build:web`, serves `apps/web/dist`, and rewrites SPA routes to `index.html`; these file-based settings override the corresponding dashboard fields.
