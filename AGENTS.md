...
- 2026-07-27: **Assets de Previsualización en README.** Se añadió un archivo SVG altamente detallado y optimizado (`text_preview.svg`) dentro de `/assets` que muestra un modelado de 1100 partículas formando "HOLA" en una tipografía serif elegante junto al icono de mano saludando ("👋") con físicas interactivas de hover simuladas mediante CSS puro, ofreciendo una demostración visual fiel directamente en el `README.md`. Se eliminó el asset de backgrounds para priorizar la fidelidad visual del motor de partículas de texto.
