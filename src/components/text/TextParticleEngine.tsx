...
import { useTextParticles } from '../../hooks/text/useTextParticles';
...
const hexToRgb = (hex: string | undefined | null): string => {
  ...
  try {
    ...
    if (hex.length === 4) {
      ...
      // Already an "r, g, b" string — pass through
      return hex;
    }
    return `${r}, ${g}, ${b}`;
  } catch {
...
export default function TextParticleEngine({
  ...
  const { updateTextTargets, textRef } = useTextParticles(
    text,
    particlesRef,
  ...
  const getParsedColors = () => {
    if (Array.isArray(particleColor)) {
      return particleColor.map(c => hexToRgb(c));
    }
    return hexToRgb(particleColor);
  };
  ...
  const initParticles = (w: number, h: number, currentDensity: number = 1) => {
    ...
    for (let i = 0; i < count; i++) {
      const p = new Particle(w, h, parsedColors);
  ...
  useEffect(() => {
    if (particlesRef.current.length > 0) {
      ...
      particlesRef.current.forEach(p => {
        p.baseColor = Array.isArray(parsedColors)
  ...
  useEffect(() => {
    if (particlesRef.current.length > 0) {
      particlesRef.current.forEach(p => { p.sizeMultiplier = particleSize; });
    }
  }, [particleSize]);

  useEffect(() => {
    if (particlesRef.current.length > 0) {
      particlesRef.current.forEach(p => { p.easeMultiplier = particleEase; });
    }
  }, [particleEase]);
    ...
    if (particlesRef.current.length > 0 && canvasRef.current && containerRef.current) {
      ...
      if (targetCount > currentCount) {
        ...
        for (let i = 0; i < targetCount - currentCount; i++) {
          const p = new Particle(rect.width, rect.height, parsedColors);
          p.sizeMultiplier = particleSize;
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      for (let i = 0; i < particlesRef.current.length; i++) {
        ...
        p.draw(ctx, currentShape, timeRef.current);
      }
